...
<img width="2492" height="1596" alt="image" src="https://github.com/user-attachments/assets/25a9deb8-d63b-46ce-afb6-d0b6630fc34a" />
...
<img width="2930" height="878" alt="image" src="https://github.com/user-attachments/assets/635825ba-8370-4ba5-8d9f-32499f60ee94" />
...
### Task 6: Dynamic Provisioning (Complete Workflow)
Let's let Kubernetes create the physical disk for us and attach it to an application to verify it works.

**1. Create the Dynamic Claim (`dynamic-pvc.yaml`)**
```yaml
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: dynamic-pvc
spec:
  storageClassName: standard # Use the default class name from Task 5
  accessModes:
    - ReadWriteOnce
  resources:
    requests:
...
**2. Create the Consumer Pod (`dynamic-pod.yaml`)**
...
<img width="2344" height="1010" alt="image" src="https://github.com/user-attachments/assets/1efdb5d6-e1aa-4b70-aea4-e26d701cb0d1" />

**3. Apply, Write Data, and Verify**
Apply both the claim and the Pod. *(Note: Because `kind` uses the `WaitForFirstConsumer` binding mode, the PV will only be created once the Pod is deployed).*
...
# 2. Write data to the dynamic volume and read it back
...
<img width="2444" height="1352" alt="image" src="https://github.com/user-attachments/assets/487af312-5e3b-4b66-a6c2-0f2e949443ff" />


**Verification:**

* **Does the file contain data?** Yes, running the `cat` command will successfully output `Dynamic Storage Test`.
* **How many PVs exist now?** You will see **two** PVs.
* **Which was manual, which was dynamic?** The `manual-pv` (1Gi) is the manual one we statically provisioned. The new one (2Gi) with the long, randomized UUID name is the dynamic one automatically generated by Kubernetes to satisfy our claim!
...
<img width="2580" height="392" alt="image" src="https://github.com/user-attachments/assets/e6516159-be43-4327-9096-7ec1a7802c59" />
...
<img width="2064" height="152" alt="image" src="https://github.com/user-attachments/assets/596b7669-83fe-4a08-acfa-18374a3c8cf6" />
